...
def get_market_data(ticker, start, end, interval="5m"):
    ...
    if is_intraday:
        ...
        if requested_start < limit:
            logger.warning("Clamping start from %s to %s (yfinance 60-day limit)", requested_start, limit)
    ...
    logger.info("Effective requested range: %s to %s", requested_start, requested_end)

    # ── If no cache, download fresh ──────────────────────────────────────────
    if df is None:
        logger.info("No cache found. Downloading full data")
        ...
        logger.info("Cache found. Checking coverage")
        df["datetime"] = pd.to_datetime(df["datetime"], utc=True)
        ...
        if is_intraday:
            ...
            if purged > 0:
                logger.info("Purged %s stale rows older than 60 days from cache", purged)
                save_to_parquet(df, ticker, effective_interval)
        ...
        if needs_update:
            logger.info("Requested range outside cache. Updating cache")
            ...
            logger.info("Downloading: %s -> %s", download_start, download_end)
            ...
            logger.info("Requested range fully covered by cache")
    ...
    logger.info("Returned %s rows for interval=%s", len(df), original_interval)
